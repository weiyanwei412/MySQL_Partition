...
def get_all_day_v2():
    d1 = datetime.date(2018, 04, 01)
    d2 = datetime.date(2018, 06, 01)
    delta = d2 - d1
    print delta.days
    # 3
...
def tb_partition_sql():
    ...
    :return:
    """
    #ALTER TABLE sms_arrive ADD PARTITION (PARTITION p20190101 VALUES LESS THAN (TO_DAYS('2019-01-02')));
    #sql = """PARTITION p%s VALUES LESS THAN (TO_DAYS('%s')) ENGINE = InnoDB,"""
    #sql = """ALTER TABLE SMS_ARRIVE ADD PARTITION (PARTITION p%s VALUES LESS THAN (TO_DAYS('%s')));"""
    sql = """ALTER TABLE %s ADD PARTITION (PARTITION p%s VALUES LESS THAN (TO_DAYS('%s')));"""
